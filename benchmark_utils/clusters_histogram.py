...
threshold_activations = get_threshold(
    [
        get_template_img(DATASET, method)
...
threshold_weights = get_threshold(
    [
        get_avg_weights_img(DATASET, method)
        for method in METHODS
    ]
)
...
for methods in METHODS:
    ...
    data_template_dict[methods] = template_data[np.abs(template_data) > 0].flatten()
    data_weights_dict[methods] = weights_data[np.abs(weights_data) > 0].flatten()
    
    # Store the clusters sizes in a dictionary
    clusters_template_dict[methods] = clusters_sizes(template_img, idx=19, threshold=threshold_activations)
    clusters_weights_dict[methods] = clusters_sizes(weights_img, idx=4, threshold=threshold_weights)


# Convert dictionaries to DataFrames
clusters_template_df = pd.DataFrame.from_dict(clusters_template_dict, orient="index").T
clusters_weights_df = pd.DataFrame.from_dict(clusters_weights_dict, orient="index").T

original_palette = sns.color_palette("Paired")
shifted_palette = [original_palette[1], original_palette[5], original_palette[3]]
