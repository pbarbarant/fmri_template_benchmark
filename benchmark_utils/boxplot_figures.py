...
def get_results_dataframe(data_path: Path, n_parcels=400) -> pd.DataFrame:
    ...
    df = df[~df["task_name"].str.contains("Math")]
    ...
    return df.sort_values(
        ["subjects", "dataset_name", "task_name", "solver_name"]
    )
...
def create_accuracy_plot(data: pd.DataFrame, figsize: tuple = (14, 7)):
    fig, ax = plt.subplots(figsize=figsize)
    # Use a better color palette
    palette = sns.color_palette(
        "Set1", n_colors=len(data["solver_name"].unique())
    )
...
fig = create_accuracy_plot(df, figsize=(11, 5))
fig.savefig(
