...
# Get the list of all .gii files recursively
contrast_paths = list(aligned_dataset_paths.glob("**/*_left.gii"))
# Remove the coefs_ prefix
contrast_paths = [
    contrast_path
    for contrast_path in contrast_paths
    if "coefs_" not in contrast_path.stem
]
# %%
...
def plot_contrast(
    ...
    **kwargs,
    ...
    # Load the contrast
    contrast_name = contrast_path.stem[:-5]
    surface_map = surface.load_surf_data(
        contrast_path.parent / f"{contrast_name}_{hemi}.gii"
    )
...
def generate_template_figure(contrast_path, vmin=-1, vmax=1):
    # Create the figure and axes with a specific size
    fig = plt.figure(figsize=(3, 4))
    grid_spec = gridspec.GridSpec(3, 2, figure=fig)
    ...
    ax = fig.add_subplot(grid_spec[1, 0], projection="3d")
    plot_contrast(
        ax,
        contrast_path,
        hemi="left",
    ...
    ax = fig.add_subplot(grid_spec[1, 1], projection="3d")
    plot_contrast(
        ax,
        contrast_path,
        hemi="right",
    ...
    ax = fig.add_subplot(grid_spec[2, 0], projection="3d")
    plot_contrast(
        ax,
        contrast_path,
        hemi="right",
    ...
    ax = fig.add_subplot(grid_spec[2, 1], projection="3d")
    plot_contrast(
        ax,
        contrast_path,
        hemi="left",
    ...
    subject_id = contrast_path.parent.name
    contrast_name = contrast_path.stem
    fig.suptitle(f"{subject_id} - {contrast_name}")
    fig.tight_layout()
    output_path = contrast_path.parent / f"{contrast_name}.pdf"
...
# Run in a loop
# for contrast_path in contrast_paths:
#     print(f"Generating individual figure for {contrast_path}")
#     generate_template_figure(contrast_path)
#     # print(f"Generating weights figure for {dataset_path.name}")
#     # generate_weights_figure(dataset_path)

# Run in parallel
Parallel(n_jobs=10)(
    delayed(generate_template_figure)(contrast_path)
    for contrast_path in contrast_paths
)
